...
bool allFinite(const canonical::MetricSample &sample) {
  ...
  return sample.defined && std::isfinite(sample.radius) &&
         finiteRange(sample.covariant) &&
         finiteRange(sample.contravariant) && finiteRange(sample.derivative) &&
         finiteRange(sample.secondDerivative);
}
...
CanonicalKerrSchildMetric::CanonicalKerrSchildMetric(const double mass,
                                                     const double spin)
    : engine_(mass, spin) {}

std::string_view CanonicalKerrSchildMetric::name() const noexcept {
  return "Kerr (canonical Slang Kerr-Schild)";
}

std::string_view CanonicalKerrSchildMetric::chartName() const noexcept {
  return ChartTraits<KerrSchildCartesianChart>::name;
}

std::type_index CanonicalKerrSchildMetric::chartType() const noexcept {
  return std::type_index(typeid(KerrSchildCartesianChart));
}

bool CanonicalKerrSchildMetric::isDefined(
...
CanonicalKerrSchildMetric::sample(const Coordinates &coordinates) const {
  if (!coordinates.allFinite()) {
    throw std::domain_error("canonical Kerr-Schild point must be finite");
  }
  auto result = engine_.metric(toArray(coordinates));
  if (!(result.radius > 0.0) || !allFinite(result)) {
    throw std::domain_error(
        "canonical Kerr-Schild metric is undefined at this chart point");
  }
  return result;
}

CovariantMetric<double, Metric::dimension>
CanonicalKerrSchildMetric::covariantMetric(
  ...
  for (int mu = 0; mu < dimension; ++mu) {
    for (int nu = 0; nu < dimension; ++nu) {
      result(mu, nu) = evaluated.covariant[static_cast<std::size_t>(mu * 4 + nu)];
    }
  }
  return result;
}
...
CanonicalKerrSchildMetric::metricJet(const Coordinates &coordinates) const {
  const auto evaluated = sample(coordinates);
...
CanonicalKerrSchildMetric::metricSecondJet(
    const Coordinates &coordinates) const {
  const auto evaluated = sample(coordinates);
